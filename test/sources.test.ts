...
test("cline: standalone CLI task — tool_result blocks dropped", () => {
  const ss = cline.sessions(join(FX, "cline/tasks"));
  assert.equal(ss.length, 1);
  const s = ss[0];
  assert.equal(s.platform, "cline");
  assert.equal(s.startedAt, 1757601600000);
  const tx = texts(s);
  assert.match(tx, /fix the build script/);
  assert.match(tx, /missing API_BASE/);
  assert.doesNotMatch(tx, /BUILD OK/);
  assert.equal(s.turns.length, 2);
});
...
test("cursor: agent-transcripts jsonl (cursor-agent CLI)", () => {
  ...
  assert.equal(ss.length, 1);
  const s = ss[0];
  assert.equal(s.platform, "cursor");
  assert.equal(s.cwd, "/Users/test/myapp");
  const tx = texts(s);
  assert.match(tx, /retry to the queue consumer/);
  assert.doesNotMatch(tx, /session ended/);
  assert.equal(s.turns.length, 2);
});
